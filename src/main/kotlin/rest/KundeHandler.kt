...
import de.hska.kunde.rest.hateoas.ListKundeModelAssembler
...
class KundeHandler(
    ...
    private val listModelAssembler: ListKundeModelAssembler
) {
    /**
            ...
            .flatMap { kunden ->
                if (kunden.isEmpty()) {
                    ...
                    if (queryParams.keys.contains("email")) {
                        val kundeModel = modelAssembler.toModel(kunden[0], request.exchange())
                        ok().body(kundeModel)
                    } else {
                        val kundenModel = kunden.map { kunde ->
                            listModelAssembler.toModel(kunde, request)
                        }
                        ok().body(kundenModel.toMono())
                    }
                }
            }
    }
